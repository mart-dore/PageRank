...
#page rank pour une matrice d'adjacence A est un dumping factor beta
...
""" Pour avoir l'evolution du nombre d'iter en fonction de beta
damping_factors = [0.1,0.3,0.5,0.75,0.98]

for Beta in damping_factors:
    PG, n_iter = page_rank(Graphe, Beta)
    print("\n nombre d'itération : " + str(n_iter) + "\n")
"""
...
if (int(q1) == 2):
    ...
    PG, n_iter = personalized_page_rank(Graphe,0.8,get_index_w_name(list_of_names))
    print("Nombre d'itération : "+str(n_iter)+"\n")
    dix_plus_vu()
